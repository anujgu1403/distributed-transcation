...
public class InventoryListener {
	...
	public void onOrderCreate(CustomEvent customEvent) {
		if (null!= customEvent && customEvent.getEvent().equalsIgnoreCase(OrderServiceConstants.ORDER_CREATED_EVENT)) {
			...
			if(clientResponse.getCode().equals("200")) {
				//Raise event to update payment
				applicationEventPublisher.publishEvent(new CustomEvent(this, customEvent.getOrderId(),customEvent.getProductId(), customEvent.getQuantity(), customEvent.getAmount(), OrderServiceConstants.INVENTORY_UPDATED));
			}else {
				//raise event to rollback order tansaction
				applicationEventPublisher.publishEvent(new CustomEvent(this, customEvent.getOrderId(),customEvent.getProductId(), customEvent.getQuantity(), customEvent.getAmount(), OrderServiceConstants.INVENTORY_UPDATION_FAILED));
			}
		}
